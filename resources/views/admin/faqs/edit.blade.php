...
@section('content')
    <div class="content-wrapper">
        ...
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <form class="forms-sample" method="POST" action="{{ route('faqs.update', $data['faq']->id) }}"
                        ...
                        <div class="form-group col-sm-12">
                            ...
                            <textarea class="form-control" id="answer" name="answer" placeholder="Answer">{{ old('answer', $data['faq']->answer) }}</textarea>
                            @error('answer')
                                <p style="color: red">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="form-group col-sm-12">
                            ...
                            <select class="form-control" id="is_active" name="is_active">
                                <option value="1"
                                    {{ old('is_active', $data['faq']->is_active) == 1 ? 'selected' : '' }}>Yes</option>
                                <option value="0"
                                    {{ old('is_active', $data['faq']->is_active) == 0 ? 'selected' : '' }}>No</option>
                            </select>
                            @error('is_active')
                                <p style="color: red">{{ $message }}</p>
                            @enderror
